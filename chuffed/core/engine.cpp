...
#ifdef HAS_PROFILER
bool doProfiling() {
    return so.print_nodes || (profilerConnector != nullptr && profilerConnector->connected());
}
  ...
  switch (rewindStyle) {
    ...
    case REWIND_SEND_SKIPPED:
      ...
      while (nodepath.size() > decisionLevelTip[newDecisionLevel]) {
        ...
        nextnodeid++;
        int parent =
            (nodepath.size() == 0) ? (-1) : (nodepath[nodepath.size() - 1]);
        int myalt =
            (altpath.size() == 0) ? (-1) : (altpath[altpath.size() - 1]);
...
#ifdef HAS_PROFILER
        if (doProfiling()) {
            profilerConnector.createNode({nodeid, restartCount, 0},
                                         {parent, restartCount, 0},
                                         myalt, 0, SKIPPED).send();
        ...
        Conflict:
...
#ifdef HAS_PROFILER
                if (doProfiling()) {
                  sendNode(profilerConnector->createNode({nodeid, restartCount, 0},
                                                         {parent, restartCount, 0},
                                                         myalt, 0, FAILED)
            ...
            if (so.lazy) {
                ...
                if (doProfiling()) {
                    ...
                    if (doProfiling()) {
                      profilerConnector->createNode(
                        {nodeid, restartCount, 0},
                        {parent, restartCount, 0},
                        myalt, 0, FAILED)
                          .set_label(mostRecentLabel)
                          .set_nogood(ss.str())
                          ...
                          .set_info(contribString.str()).send();
                ...
                if (doProfiling()) {
                  profilerConnector->createNode(
                    {nodeid, restartCount, 0}, {parent, restartCount, 0}, myalt, 0, FAILED)
                      .set_label(mostRecentLabel).send();
                  // .set_decision_level(previousDecisionLevel)
                  mostRecentLabel = "";
                }
#endif
            ...
            if (!di) {
                ...
                if (fzs != NULL) {
                    ...
                    if (doProfiling()) {
                      profilerConnector->createNode(
                        {nodeid, restartCount, 0}, {parent, restartCount, 0},
                        myalt, 0, SOLVED)
                          .set_label(mostRecentLabel)
                          .set_info(s.str()).send();
                          // .set_decision_level(previousDecisionLevel)
                    ...
                    if (doProfiling()) {
                      profilerConnector->createNode(
                        {nodeid, restartCount, 0}, {parent, restartCount, 0}, myalt, 0, SOLVED)
                          .set_label(mostRecentLabel).send();
                          // .set_decision_level(previousDecisionLevel)
                    }
#endif
