...
//#endif
  ...
  enqueue(out_learnt[0], (so.bin_clause_opt && c->size() == 2) ? Reason(out_learnt[1], prop_id) : Reason(c, prop_id));
...
void SAT::getLearntClause(int nodeid, std::set<int>& contributingNogoods) {
  ...
  while (true) {
    ...
    for (int j = (p == lit_Undef) ? 0 : 1; j < c.size(); j++) {
      ...
      if (!seen[x]) {
        ...
        if (isCurLevel(x)) {
          ...
          ss << " " << getLitString(toInt(~q)) << ":" << reason[var(q)].cid;
    ...
    //std::string cids = ss.str();
    //if(cids.size() > 0) 
    //  std::cerr << "Involved:" << cids << "\n";
